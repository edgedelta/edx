...
func extractRows(v any, depth int) []map[string]any {
	...
	case []any:
		...
		return rows
	case map[string]any:
		for _, key := range []string{"items", "matches", "stats", "records", "data", "agents", "deployments", "spans", "traces", "monitors", "dashboards", "options", "rehydrations", "potential_rehydrations", "workflows", "executions", "steps"} {
